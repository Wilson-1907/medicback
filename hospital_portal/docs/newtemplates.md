...
## Batch 3 — HPV failed sample (insufficient sample → VIA)

**Replaces** deprecated `afya_hpv_failed_en` / `afya_hpv_failed_sw` (old retest wording).

| # | English | Kiswahili | When sent |
|---|---------|-----------|-----------|
| 24 | `afya_hpv_sample_failed_en` | `afya_hpv_sample_failed_sw` | Staff confirms **HPV failed** (insufficient sample) after VIA screening appointment is booked |

### `afya_hpv_sample_failed_en`

Variables: `{{1}}` name, `{{2}}` appointment date

```
Hello {{1}}, Your HPV test result could not be completed because the sample collected was not sufficient for testing. This does not mean that there is a problem with your health. We kindly request that you return to the health facility for VIA screening to complete your cervical screening. Appointment Date: {{2}} Thank you for choosing Afya Rafiki.
```

### `afya_hpv_sample_failed_sw`

```
Habari {{1}}, Majibu yako ya kipimo cha HPV hayakuweza kupatikana kwa sababu sampuli iliyokusanywa haikutosha kufanyiwa uchunguzi. Hii haimaanishi kuwa una tatizo la kiafya. Tafadhali rudi katika kituo cha afya kwa uchunguzi wa VIA ili kukamilisha uchunguzi wako wa kuona kama kuna mabadiliko yeyote ya mlango wa kizazi. Tarehe ya miadi (Clinic): {{2}} Asante kwa kutumia Afya Rafiki.
```
